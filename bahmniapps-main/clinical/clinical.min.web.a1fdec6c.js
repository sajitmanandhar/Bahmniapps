"use strict";
angular.module("bahmni.common.conceptSet").factory("formService", ["$http", function($http) {
    ...
    var getAllPatientForms = function(patientUuid, numberOfVisits, patientProgramUuid) {
        ...
        return $http.get(patientFormsUrl, {
            params: params
        })
    };
    return {
...
}]), angular.module("bahmni.common.services").factory("pacsService", ["$http", function($http) {
    return {
        ...
        getAccessionNumber: function(identifier) {
            if (identifier.system.indexOf("urn:bahmni:accession") < 0) return null;
            var parts = identifier.value.split("urn:oid:");
            return parts && 2 === parts.length ? parts[1] : ""
        }
    }
}]), angular.module("bahmni.common.services").factory("virtualConsultService", ["$http", "$rootScope", function($http, $rootScope) {
    return {
        launchMeeting: function(uuid, link) {
            $rootScope.$broadcast("event:launchVirtualConsult", {
                uuid: uuid,
                link: link
            })
        }
    }
}]), angular.module("bahmni.common.util").factory("transmissionService", ["$http", "$q", "$rootScope", "locationService", "$bahmniCookieStore", "$translate", "appService", "visitService", "$filter", "messagingService", function($http, $q, $rootScope, locationService, $bahmniCookieStore, $translate, appService, visitService, $filter, messagingService) {
    return {
        sendEmail: function(attachments, subject, body, emailUrl, cc, bcc) {
            var params = {
                ...
                deferred = $q.defer();
            return $http.post(emailUrl, params, {
                withCredentials: !0,
                headers: {
                ...
                200 != response.data.statusLine.statusCode ? messagingService.showMessage("error", response.data.statusLine.reasonPhrase) : messagingService.showMessage("info", response.data.statusLine.reasonPhrase), deferred.resolve(response)
            }), deferred.promise
        },
        getSharePrescriptionMailContent: function(prescriptionDetails) {
            var message = $translate.instant(Bahmni.Clinical.Constants.sharePrescriptionMailContent);
            return message = message.replace("#recipientName", prescriptionDetails.patient.name), message = message.replaceAll("#locationName", $rootScope.facilityLocation.name), message = message.replace("#locationAddress", $rootScope.facilityLocation.attributes[0] ? $rootScope.facilityLocation.attributes[0].display.split(":")[1].trim() : ""), message = message.replace("#visitDate", $filter("bahmniDate")(prescriptionDetails.visitDate))
        }
    }
    ...
    this.getAllFor = function(searchTerm, locale) {
        var url = Bahmni.Common.Constants.bahmniapiConceptUrl,
            parameters = {
                term: searchTerm,
                limit: Bahmni.Common.Constants.emrapiDiagnosisLimit
            };
        return locale && (parameters.locale = locale), $http.get(url, {
            params: parameters
        })
...
}]), angular.module("bahmni.common.domain").service("conditionsService", ["$http", function($http) {
    this.save = function(conditions, patientUuid) {
        var conditionsToBeSaved = _.reject(conditions, function(condition) {
                return null === condition.onSetDate || Number.isInteger(condition.onSetDate)
            }),
            body = _.map(conditionsToBeSaved, function(condition) {
                return {
                    uuid: condition.uuid,
                    patientUuid: patientUuid,
    ...
    this.getConditions = function(patientUuid) {
        return this.getConditionHistory(patientUuid).then(function(response) {
            var conditionHistories = response.data,
                conditions = Bahmni.Common.Domain.Conditions.fromConditionHistories(conditionHistories);
            return _.forEach(conditions, function(condition) {
                condition.activeSince = getLatestActiveCondition(conditionHistories, condition).onSetDate
            }), conditions
        })
    }
}]), angular.module("bahmni.common.orders").service("orderTypeService", ["$http", function($http) {
    var self = this;
    self.orderTypes = [], self.loadAll = function() {
        return $http.get("/openmrs/ws/rest/v1/ordertype", {
            params: {
                v: "custom:(uuid,display,conceptClasses:(uuid,display,name))"
            }
        }).then(function(response) {
            return self.orderTypes = response.data.results, self.orderTypes
        })
    }, self.getOrderTypeUuid = function(orderTypeName) {
        return _.result(_.find(self.orderTypes, {
            display: orderTypeName
        }), "uuid")
    }
    ...
    this.getPatient = function(uuid, rep) {
        return rep || (rep = "full"), $http.get(Bahmni.Common.Constants.openmrsUrl + "/ws/rest/v1/patient/" + uuid, {
            method: "GET",
            params: {
                v: rep
            },
        ...
        return offset = offset || 0, identifier = identifier || query, $http.get(Bahmni.Common.Constants.bahmniCommonsSearchUrl + "/patient/lucene", {
            method: "GET",
            params: {
                filterOnAllIdentifiers: !0,
                q: query,
                startIndex: offset,
    ...
    return {
        ...
        getFacilityVisitLocation: function() {
            var cookie = $bahmniCookieStore.get(Bahmni.Common.Constants.locationCookieName);
...
}]), angular.module("bahmni.clinical").factory("labOrderResultService", ["$http", "$q", "configurationService", function($http, $q, configurationService) {
    ...
    var sanitizeData = function(labOrderResults) {
            labOrderResults.forEach(function(result) {
                result.accessionDateTime = Bahmni.Common.Util.DateUtil.parse(result.accessionDateTime), result.hasRange = result.minNormal || result.maxNormal
            })
        },
        groupByPanel = function(accessions) {
            var grouped = [];
            return accessions.forEach(function(labOrders) {
                var panels = {},
                    accessionGroup = [];
                labOrders.forEach(function(labOrder) {
                    labOrder.panelName ? (panels[labOrder.panelName] = panels[labOrder.panelName] || {
                        accessionDateTime: labOrder.accessionDateTime,
                        orderName: labOrder.panelName,
                        tests: [],
                        isPanel: !0
                    }, panels[labOrder.panelName].tests.push(labOrder)) : (labOrder.isPanel = !1, labOrder.orderName = labOrder.testName, accessionGroup.push(labOrder))
                }), _.values(panels).forEach(function(val) {
                    accessionGroup.push(val)
                }), grouped.push(accessionGroup)
            }), grouped
        },
        flattened = function(accessions) {
            return accessions.map(function(results) {
                return _(results).map(function(result) {
                    return !0 === result.isPanel ? [result, result.tests] : result
                }).flattenDeep().value()
            })
        },
        transformGroupSort = function(results, initialAccessionCount, latestAccessionCount) {
            ...
            var accessionConfig = {
                ...
                tabularResult = new Bahmni.Clinical.TabularLabOrderResults(results.tabularResult, accessionConfig),
                accessions = _.groupBy(labOrderResults, function(labOrderResult) {
                    return labOrderResult.accessionUuid
                });
            if (accessions = _.sortBy(accessions, function(accession) {
                    return accession[0].accessionDateTime
                }), accessionConfig.initialAccessionCount || accessionConfig.latestAccessionCount) {
                var initial = _.take(accessions, accessionConfig.initialAccessionCount || 0),
                    latest = _.takeRight(accessions, accessionConfig.latestAccessionCount || 0);
    ...
    return {
        getAllForPatient: function(params) {
            var deferred = $q.defer(),
                paramsToBeSent = {};
            return params.visitUuids ? paramsToBeSent.visitUuids = params.visitUuids : (params.patientUuid || deferred.reject("patient uuid is mandatory"), paramsToBeSent.patientUuid = params.patientUuid, 0 !== params.numberOfVisits && (paramsToBeSent.numberOfVisits = params.numberOfVisits)), $http.get(Bahmni.Common.Constants.bahmniLabOrderResultsUrl, {
                method: "GET",
                ...
                var results = transformGroupSort(response.data, params.initialAccessionCount, params.latestAccessionCount),
                    sortedConceptSet = new Bahmni.Clinical.ConceptWeightBasedSorter(allTestsAndPanelsConcept),
                    resultObject = {
                        labAccessions: flattened(results.accessions.map(sortedConceptSet.sortTestResults)),
                        tabular: results.tabularResult
                    };
                resultObject.tabular.tabularResult.orders = sortedConceptSet.sortTestResults(resultObject.tabular.tabularResult.orders), deferred.resolve(resultObject)
            }), deferred.promise
        }
    }
    ...
    var getBacteriologyGroupMembers = function(encounter) {
            var addBacteriologyMember = function(bacteriologyGroupMembers, member) {
                return bacteriologyGroupMembers = member.groupMembers.length ? bacteriologyGroupMembers.concat(member.groupMembers) : bacteriologyGroupMembers.concat(member)
            };
            return encounter.extensions && encounter.extensions.mdrtbSpecimen ? encounter.extensions.mdrtbSpecimen.map(function(observation) {
                ...
                return observation.sample.additionalAttributes && observation.sample.additionalAttributes.groupMembers.forEach(function(member) {
                    ...
                    bacteriologyGroupMembers = addBacteriologyMember(bacteriologyGroupMembers, member)
                }), bacteriologyGroupMembers
            }) : []
        },
        getDefaultEncounterType = function() {
    ...
    this.search = function(visitUuid, encounterDate) {
        ...
        return params = params || {
            includeAll: !0
        }, $http.get(Bahmni.Common.Constants.bahmniEncounterUrl + "/" + encounterUuid, {
            params: params,
            withCredentials: !0
        })
...
}]), angular.module("bahmni.clinical").service("diseaseTemplateService", ["$http", "$q", "clinicalAppConfigService", function($http, $q, clinicalAppConfigService) {
    this.getLatestDiseaseTemplates = function(patientUuid, diseaseTemplates, startDate, endDate) {
        var url = Bahmni.Common.Constants.diseaseTemplateUrl,
            params = {
                patientUuid: patientUuid,
                diseaseTemplateConfigList: diseaseTemplates
            };
        params.startDate = startDate && moment(startDate).format(Bahmni.Common.Constants.ServerDateTimeFormat), params.endDate = endDate && moment(endDate).format(Bahmni.Common.Constants.ServerDateTimeFormat);
        var deferred = $q.defer();
    ...
    }, this.getObsInFlowSheet = function(patientUuid, conceptSet, groupByConcept, orderByConcept, conceptNames, numberOfVisits, initialCount, latestCount, groovyExtension, startDate, endDate, patientProgramUuid, formNames) {
        var params = {
            patientUuid: patientUuid,
    ...
    return {
        getConcept: function(params, cache) {
...
}), angular.module("bahmni.common.logging").service("auditLogService", ["$http", "$translate", "configurationService", function($http, $translate, configurationService) {
...
}]), angular.module("bahmni.clinical").factory("treatmentService", ["$http", "$q", "$compile", "$timeout", "spinner", "appService", "$rootScope", "transmissionService", "$filter", function($http, $q, $compile, $timeout, spinner, appService, $rootScope, transmissionService, $filter) {
    var createDrugOrder = function(drugOrder) {
            return Bahmni.Clinical.DrugOrder.create(drugOrder)
        },
        getActiveDrugOrdersFromServer = function(patientUuid, startDate, endDate) {
        ...
        getPrescribedAndActiveDrugOrders = function(patientUuid, numberOfVisits, getOtherActive, visitUuids, startDate, endDate, getEffectiveOrdersOnly) {
            return $http.get(Bahmni.Common.Constants.bahmniDrugOrderUrl + "/prescribedAndActive", {
                ...
                for (var key in response) response[key] = response[key].map(createDrugOrder)
            })
        },
        getConfig = function() {
        ...
        getProgramConfig = function() {
            return appService.getAppDescriptor() ? appService.getAppDescriptor().getConfigValue("program") || {} : {}
        };
    return {
        getActiveDrugOrders: function(patientUuid, fromDate, toDate) {
            var programConfig = getProgramConfig(),
                startDate = programConfig.showDetailsWithinDateRange ? fromDate : null,
                endDate = programConfig.showDetailsWithinDateRange ? toDate : null,
                deferred = $q.defer();
            return getActiveDrugOrdersFromServer(patientUuid, startDate, endDate).success(function(response) {
                var activeDrugOrders = response.map(createDrugOrder);
                deferred.resolve(activeDrugOrders)
            }), deferred.promise
        },
        getConfig: getConfig,
        getPrescribedDrugOrders: function(patientUuid, includeActiveVisit, numberOfVisits, fromDate, toDate) {
            var programConfig = getProgramConfig(),
                startDate = programConfig.showDetailsWithinDateRange ? fromDate : null,
                endDate = programConfig.showDetailsWithinDateRange ? toDate : null,
                deferred = $q.defer();
            return $http.get(Bahmni.Common.Constants.bahmniDrugOrderUrl, {
                method: "GET",
                params: {
                    patientUuid: patientUuid,
                    numberOfVisits: numberOfVisits,
        ...
        getNonCodedDrugConcept: function() {
            var deferred = $q.defer();
            return $http.get(Bahmni.Common.Constants.globalPropertyUrl, {
                method: "GET",
                params: {
        ...
        getAllDrugOrdersFor: function(patientUuid, conceptSetToBeIncluded, conceptSetToBeExcluded, isActive, patientProgramUuid) {
            var deferred = $q.defer(),
                params = {
                    patientUuid: patientUuid
                };
            return conceptSetToBeIncluded && (params.includeConceptSet = conceptSetToBeIncluded), conceptSetToBeExcluded && (params.excludeConceptSet = conceptSetToBeExcluded), void 0 !== isActive && (params.isActive = isActive), patientProgramUuid && (params.patientProgramUuid = patientProgramUuid), $http.get(Bahmni.Common.Constants.bahmniDrugOrderUrl + "/drugOrderDetails", {
                params: params,
                withCredentials: !0
            }).success(function(response) {
        ...
        voidDrugOrder: function(drugOrder) {
            var deferred = $q.defer();
            return $http.delete([Bahmni.Common.Constants.ordersUrl, "/", drugOrder.uuid].join("")).success(function(response) {
                deferred.resolve(response)
            }), deferred.promise
        },
        sharePrescriptions: function(prescriptionDetails) {
            $http.get("common/views/prescriptionPrint.html").then(function(templateData) {
                ...
                var element = $compile($("<div>" + template + "</div>"))(printScope),
                    renderAndSendPromise = $q.defer(),
                    waitForRenderAndSend = function() {
                        return printScope.$$phase || $http.pendingRequests.length ? $timeout(waitForRenderAndSend, 1e3) : (html2pdf().from(element.html()).outputPdf().then(function(pdfContent) {
                            var attachments = [{
                                ...
                                subject = "Prescription for consultation at " + $rootScope.facilityLocation.name + " on " + $filter("bahmniDate")(prescriptionDetails.visitDate),
                                body = transmissionService.getSharePrescriptionMailContent(prescriptionDetails),
                                emailUrl = appService.getAppDescriptor().formatUrl(Bahmni.Common.Constants.sendViaEmailUrl, {
                                    patientUuid: prescriptionDetails.patient.uuid
                                });
                            transmissionService.sendEmail(attachments, subject, body, emailUrl, [], [])
                        }), renderAndSendPromise.resolve(), printScope.$destroy()), renderAndSendPromise.promise
                    };
